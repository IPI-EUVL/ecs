...
class WriteClient:
    ...
    def set_value(self, value):
        if self.__remote_kv is not None:
            self.__remote_kv.try_set(value).then(self.__on_ok).catch(self.__on_fail)

    def __on_ok(self, value):
        self.__last_ret = value

    def __on_fail(self, state, reason):
        print("Set failed:", state, reason)
        self.__last_ret = (state, reason)
    ...
    def did_set(self):
        return self.__last_ret is not None

def main(args: argparse.Namespace):
    m_client = WriteClient(args.name, args.sys, args.key)
    hz = args.hz if args.hz is not None else 1.0

    try:
        while m_client.ok():
            m_client.set_value(args.value.encode("utf-8"))
            time.sleep(1 / hz)
            if m_client.did_set():
                time.sleep(1)
                break
