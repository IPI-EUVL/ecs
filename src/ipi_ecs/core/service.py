...
class ServiceHandler(win32serviceutil.ServiceFramework):
    _svc_name_ = "ipi-ecs-default"
    _svc_display_name_ = "ipi-ecs Default Service Handler"
    _svc_description_ = "Default service template for ipi-ecs"

    def __init__(self, args):
        self.__logger_sock = None
        self.__logger = None
        self.__m_server = None
        _log_to_file(f"{self._svc_name_}.__init__ sys.executable={sys.executable}")
...
def main(servicer_class):
    if len(sys.argv) == 1:
        _log_to_file("main: StartServiceCtrlDispatcher")
        servicemanager.Initialize()
        servicemanager.PrepareToHostSingle(servicer_class)
        servicemanager.StartServiceCtrlDispatcher()
    else:
        # Called from command line with parameters (install, start, etc.)
        rc = win32serviceutil.HandleCommandLine(servicer_class)
